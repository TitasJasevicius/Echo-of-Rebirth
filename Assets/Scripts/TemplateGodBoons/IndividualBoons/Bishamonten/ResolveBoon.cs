...
public class ResolveBoon : MonoBehaviour
{
  ...
  public void ApplyBoon(PlayerResources playerResources)
  {
    ...
    if (Activated == false && playerResources.money >= DiscountedPrice)
    {
      playerResources.IncreaseBaseDamageReduction(DamageReductionMultiplier);
      Activated = true;
      playerResources.money -= DiscountedPrice;
      playerResources.resolve = true; //update players resolve boon status
      Debug.Log($"Damage reduction increased by {DamageReductionMultiplier}%.");
    }
  }
  public void SetDamageReductionMultiplier(float percentage)
  {
    DamageReductionMultiplier = percentage;
  }
  public int GetPrice()
  {
